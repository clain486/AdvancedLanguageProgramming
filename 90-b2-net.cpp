...
***************************************************************************/
int game_progress(mto10_net_tools& client, coordinate* p_co)
{
	...
	while (1) {
		...
		row = 'A';
		col = 1;
		int hang_num = client.get_row(), * p_hang_num = &hang_num;
		int lie_num = client.get_col(), * p_lie_num = &lie_num;
		const char* p_map = client.get_map();
		for (int i = 1; i < *p_hang_num + 1; i++) {
			for (int j = 1; j < *p_lie_num + 1; j++, n++) {
				if (*(p_map + n) >= 48 && *(p_map + n) <= 57) {
					p_co->array[i][j] = (*(p_map + n) - 48);
				}
				else if (*(p_map + n) >= 65 && *(p_map + n) <= 70) {
					p_co->array[i][j] = (*(p_map + n) - 55);
				}
				else if (*(p_map + n) >= 97 && *(p_map + n) <= 102) {
					p_co->array[i][j] = (*(p_map + n) - 87);
				}
			}
		}
		for (int i = *p_hang_num; i > 0; i--) {
			for (int j = 1; j < *p_lie_num + 1; j++) {
				if (same_choose(p_hang_num, p_lie_num, i, j, p_co) != 0) {
					row = (char)(i+64);
					col = j - 1;
					break;
				}
...
{
	const char* my_no = "2151140";		//本人学号
	const char* my_pwd = "wqms20011013.";	//本人密码
	...
	wanted_row = *p_hang_num;
	wanted_col = *p_lie_num;
	...
	if (game_progress(client,p_co) < 0) {
		client.close();
		return -1;
	}
